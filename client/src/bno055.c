...
static void read_registers(uint8_t reg, uint8_t* buf, uint8_t len) {
    i2c_write_blocking(I2C_ID, I2C_ADDR, &reg, 1, true);
...
void bno055_set_mode(uint8_t mode) {
    write_register(0x3D, 0x0C);
}

void bno055_read_accel(bno055_accel_t* acc) {
    uint8_t buf[6];
    read_registers(0x08, buf, 6);
